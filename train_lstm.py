...
if __name__ == "__main__":
    ...
    results_df.to_csv('results_lstm_summary.csv', index=False)
    print("\n结果已保存到 results_lstm_summary.csv")

    print("\n训练完成！LSTM 模型已保存:")
    for f in sorted(os.listdir('.')):
        if f.startswith('model_lstm_') and f.endswith('.pt'):
            print(f"  - {f}")
